...
export function Shows() {
  ...
  return (
    <section id="shows" className="scroll-mt-20 bg-coal py-20 sm:py-28">
      <div className="shell">
        <SectionHeading
          kicker="On Tour"
        ...
        {upcoming.length > 0 ? (
          <ul className="mt-12 border-t border-edge">
            {upcoming.map((show) => {
              ...
              return (
                <li
                  key={show.id}
                  className="group flex flex-col gap-4 border-b border-edge py-6 transition-colors hover:bg-bone/[0.03] sm:flex-row sm:items-center sm:gap-8 sm:py-7"
                >
                  {/* Date */}
                  <div className="flex shrink-0 items-baseline gap-3 sm:w-32 sm:flex-col sm:items-start sm:gap-0">
                    ...
                    <span className="text-sm text-smoke sm:mt-1">{d.year}</span>
                  </div>

                  {/* Venue */}
                  <div className="min-w-0 flex-1">
                    <h3 className="text-2xl text-bone sm:text-3xl">
                      {show.venue}
                    </h3>
                  ...
                  <div className="shrink-0">
                    {show.ticketUrl ? (
                      <a
                        ...
                        className="inline-flex items-center gap-2 rounded-full border border-edge px-5 py-2.5 text-sm font-semibold uppercase tracking-wider text-bone transition-colors group-hover:border-ember group-hover:text-ember"
                      >
                        Tickets
          ...
          <div className="mt-12 rounded-2xl border border-edge bg-ink/40 p-10 text-center">
            ...
            <p className="mx-auto mt-3 max-w-md text-smoke">
              We are always cooking up the next one. Follow along to hear about
              new dates the moment they drop.
            </p>
